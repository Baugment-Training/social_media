...
BAUGMENT.views.import = (function () {
  ...
  function buildRecord(raw, index) {
    const rec = {
      id: U.uid('ana'), platform: '', account: '', account_id: null, post_id: '', post_url: '', caption: '',
    ...
    /* Resolve the handle against the accounts registered in Settings rather
       than a hardcoded default. Where the file names a channel we match it;
       where it doesn't and the platform has exactly one account, that's the
       only answer it can be. Two or more and we say so instead of guessing —
       a wrongly attributed post is worse than a blank field. */
    if (rec.platform) {
      const onPlatform = store.state().accounts.filter((a) => a.platform === rec.platform);
      const named = rec.account
        ? onPlatform.find((a) => (a.handle || '').toLowerCase() === String(rec.account).trim().toLowerCase() ||
                                 (a.name || '').toLowerCase() === String(rec.account).trim().toLowerCase())
        : null;
      if (named) { rec.account = named.handle; rec.account_id = named.id; }
      else if (!rec.account && onPlatform.length === 1) {
        rec.account = onPlatform[0].handle;
        rec.account_id = onPlatform[0].id;
      } else if (!rec.account && onPlatform.length > 1) {
        problems.push(onPlatform.length + ' ' + S.platform(rec.platform).name +
          ' accounts are registered and this file has no channel column — left blank');
      }
    }
  ...
  function sampleTemplate() {
    /* Use the handles this workspace has actually registered, so the example
       rows import cleanly instead of inventing an account nobody owns. */
    const handle = (platform, fallback) => {
      const a = store.state().accounts.find((x) => x.platform === platform);
    ...
    const rows = [
      ['2026-07-18', handle('linkedin', 'baugment'), 'LinkedIn', 'https://linkedin.com/feed/update/example1', 'Why completion rate is a vanity metric.', 'Carousel', '24800', '19400', '1420', '0', '96', '118', '0', '620', '0', 'LinkedIn Thought Leadership', 'Measurement'],
      ['2026-07-19', handle('instagram', '@baugment'), 'Instagram', 'https://instagram.com/p/example2', 'Storyboarding a module in 60 seconds.', 'Reel', '18100', '15600', '940', '760', '58', '64', '148', '190', '16200', 'Storyboarding Shorts', 'Instructional Design'],
      ['2026-07-20', handle('youtube', '@baugment'), 'YouTube', 'https://youtube.com/watch?v=example3', 'Building a learning analytics dashboard from scratch.', 'Video', '9400', '7800', '520', '410', '38', '26', '0', '210', '7600', '', 'Measurement']
    ];
