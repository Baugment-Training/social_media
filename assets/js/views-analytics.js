...
BAUGMENT.views.analytics = (function () {
  ...
  function blankPost() {
    ...
    const platform = live ? live.id : 'instagram';
    /* One account on the platform means there is nothing to choose — preselect
       it rather than making someone pick the only option. */
    const only = accountsFor(platform).length === 1 ? accountsFor(platform)[0] : null;
    return {
      id: U.uid('ana'),
      platform,
      account_id: only ? only.id : null, account: only ? only.handle : '',
      post_id: '', post_url: '', caption: '',
  ...
  /* The handle is picked from the accounts registered in Settings, filtered to
     the platform currently selected — choosing Instagram should never offer a
     LinkedIn page. "Other…" keeps a free-text escape hatch, so an imported
     handle that was never registered survives an edit, and nobody is blocked
     when an account hasn't been set up yet. */
  ...
  function accountField(r) {
    const list = accountsFor(r.platform);
    const known = list.some((a) => a.handle === r.account);
    const custom = !!r.account && !known;
    const pf = S.platform(r.platform).name;

    const options =
      '<option value=""' + (!r.account ? ' selected' : '') + '>— none —</option>' +
      list.map((a) => '<option value="' + esc(a.handle) + '"' + (r.account === a.handle ? ' selected' : '') + '>' +
        esc(a.handle) + (a.name && a.name !== a.handle ? ' · ' + esc(a.name) : '') + '</option>').join('') +
      '<option value="__other"' + (custom ? ' selected' : '') + '>Other…</option>';

    const hint = list.length
      ? 'From Settings → Platforms → Connected accounts.'
      : 'No ' + pf + ' account is registered yet — add one under Settings → Platforms.';

    return '<label class="field"><span class="field__label">Account handle</span>' +
      '<select class="select" data-account-pick>' + options + '</select>' +
      '<input class="input" data-f="account" data-account-other value="' + esc(r.account || '') + '" ' +
      'placeholder="@handle" style="margin-top:8px' + (custom ? '' : ';display:none') + '">' +
  ...
  function editPost(record, onSaved) {
    ...
    actions.push({
      ...
      onClick: (bodyEl, close) => {
        ...
        /* Match the handle that was actually chosen. Falling back to "the
           first account on this platform" quietly mislabels a post the moment
           there are two Instagram accounts. */
        const options = accountsFor(r.platform);
        const chosen = options.find((a) => a.handle === r.account) ||
          (!r.account && options.length === 1 ? options[0] : null);
        if (chosen) { r.account = chosen.handle; r.account_id = chosen.id; }
    ...
    /* The picker writes through to the text input, which stays the single
       source of truth so `harvest` needs no special case. */
    const pick = m.body.querySelector('[data-account-pick]');
    const other = m.body.querySelector('[data-account-other]');
    ...
    /* Switching platform changes which metrics exist and which accounts are
       offered, so redraw. A handle belonging to the old platform is dropped
       rather than carried across, where it would read as a custom entry. */
    m.body.querySelector('[data-f="platform"]').addEventListener('change', () => {
      harvest(m.body);
      const options = accountsFor(r.platform);
      if (!options.some((a) => a.handle === r.account)) {
        /* Preselect where there's only one possible answer, blank otherwise. */
        r.account = options.length === 1 ? options[0].handle : '';
        r.account_id = options.length === 1 ? options[0].id : null;
      }
      m.close();
      editPost(r, onSaved);
    });
