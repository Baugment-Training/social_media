...
BAUGMENT.views.media = (function () {
  ...
  /* Seeded items have no binary behind them, so the thumbnail is drawn from
     the record: a royal-blue gradient keyed to the file's stored hue. */
  function thumb(m) {
    if (m.dataUrl) return '<img src="' + esc(m.dataUrl) + '" alt="' + esc(m.name) + '">';
    const h = m.hue || 224;
    return '<svg viewBox="0 0 120 90" preserveAspectRatio="none" aria-hidden="true">' +
      ...
      /* four modules on the grid, the same motif as the sign-in stage */
      [0, 1].map((r) => [0, 1].map((c) =>
  ...
  function preview(m) {
    BAUGMENT.ui.modal({
      title: m.name,
      body: '<div style="border-radius:12px;overflow:hidden;border:1px solid var(--line);margin-bottom:16px;max-height:46vh">' +
        (m.dataUrl && m.kind === 'video'
          ? '<video src="' + esc(m.dataUrl) + '" controls style="width:100%"></video>'
          : thumb(m)) + '</div>' +
        '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px">' +
        [['Kind', m.kind], ['Category', m.category], ['Size', (m.size / 1048576).toFixed(2) + ' MB'],
         ['Dimensions', m.width + '×' + m.height], ['Uploaded', U.fmt.date(m.uploaded)], ['Tags', m.tags.join(', ')]]
          .map((f) => '<div><div class="tiny mute">' + esc(f[0]) + '</div><div class="tiny">' + esc(f[1]) + '</div></div>').join('') +
        '</div>',
      actions: [
        { label: 'Remove', variant: 'danger', keepOpen: true, onClick: async (b, close) => {
            const ok = await BAUGMENT.ui.confirm('Remove ' + m.name + '?', 'It disappears from the library and from any post that used it as a thumbnail.', 'Remove');
            if (!ok) return false;
  ...
  function ingest(files) {
    let added = 0;
    files.forEach((file) => {
      const kind = file.type.indexOf('video') === 0 ? 'video' : file.type.indexOf('image') === 0 ? 'image' : 'document';
      const reader = new FileReader();
      reader.onload = () => {
        store.upsert('media', {
          id: U.uid('med'), name: file.name, kind,
          tags: ['uploaded'], category: 'Uncategorised',
          size: file.size, width: 0, height: 0, hue: 24,
          uploaded: U.iso(new Date()),
          /* Held for this session only — see the note in the README. */
          dataUrl: kind === 'document' ? null : reader.result
        });
        added++;
        if (added === files.length) {
          BAUGMENT.ui.toast('Added to library', added + ' file' + (added === 1 ? '' : 's'));
          BAUGMENT.app.render();
        }
      };
      if (kind === 'document') reader.onload();
  ...
  function render(el) {
    ...
    const bar = '<div class="row no-print" style="margin-bottom:16px">' +
      ...
      '<div class="search" style="max-width:280px">' + BAUGMENT.icon.render('search', 15) +
    ...
    const zone = '<div class="dropzone no-print" id="mediaDrop" style="margin-bottom:16px;padding:26px">' +
      '<div class="dropzone__icon">' + BAUGMENT.icon.render('upload', 34) + '</div>' +
      '<div style="font-weight:600">Drop images, videos or documents here</div>' +
      '<p class="tiny mute" style="margin-top:6px">Or click to browse. Files stay in this browser session.</p>' +
    ...
    const grid = items.length
      ? '<div class="media-grid">' + items.map((m) =>
          '<article class="media" data-id="' + m.id + '">' +
            '<div class="media__thumb">' + thumb(m) + '<span class="media__kind">' + esc(m.kind) + '</span></div>' +
    ...
    search.addEventListener('input', U.debounce(() => { q = search.value; render(el); const s = document.getElementById('mediaSearch'); if (s) { s.focus(); s.setSelectionRange(s.value.length, s.value.length); } }, 260));
    ...
    input.addEventListener('change', () => { if (input.files.length) ingest(Array.from(input.files)); });
