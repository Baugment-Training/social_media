...
| **Media Library** | Grid with preview, search, tags, categories and kind filters. Drag files in to add them. |
...
2. **Run the schema.**
   ...
   It creates eleven tables, indexes them, and turns on Row Level Security. Safe to run more than once. There is no `migrations/` folder — this is a new project, so everything is in the one file.
...
### Two things that do *not* sync

- **Uploaded media files.** Media *records* sync; the image bytes are still session-only. The schema has a `storage_path` column ready for a Supabase Storage bucket, but the upload path isn't wired yet.
...
Uploaded media is held as data URLs **for the session only**, since a handful of images would blow past the 5 MB storage quota. Seeded media items are drawn from their record rather than stored as images.
